...
from create_datebase import create_db
from crub import Friends, Friend_Computer, Computer, Processor, Motherboard
...
class App(ttk.Window):
    ...
    def create_widget(self):
        ...
        ttk.Label(
            ...
            text='Заказы:',
            font=FONT
        ).grid(row=0, column=1, sticky='nw', padx=10, pady=5)
        ...
        self.table_friends.heading('id', text='ID Клиента', anchor=tk.W)
        self.table_friends.heading('name', text='Имя', anchor=tk.W)  # Изменено на "Банк"
        self.table_friends.heading('phone', text='Телефон', anchor=tk.W)
        self.table_friends.column('id', width=100, stretch=False)
        self.table_friends.column('name', width=150, stretch=False)
        self.table_friends.column('phone', width=150, stretch=False)

        # Ползунок прокрутки для таблицы клиентов
        ...
        self.table_friends.grid(row=1, column=0, sticky='nsew', padx=10, pady=5)
        self.scr_friends.grid(row=1, column=0, sticky='nse', padx=(0, 10), pady=5)

        # Таблица заказов
        self.table_computers = ttk.Treeview(
            self,
            columns=('id', 'status', 'processor', 'motherboard', 'corpus', 'memory'),
            show='headings',
            bootstyle='danger'
        )
        ...
        self.table_computers.heading('corpus', text='Цвет корпуса', anchor=tk.W)
        self.table_computers.heading('memory', text='Память', anchor=tk.W)

        # Настройка ширины столбцов
        for col in ('id', 'status'):
            self.table_computers.column(col, width=80, stretch=False)
        for col in ('processor', 'motherboard', 'corpus', 'memory'):
            self.table_computers.column(col, width=120, stretch=False)

        # Ползунок прокрутки для таблицы заказов
        ...
        self.table_computers.grid(row=1, column=1, sticky='nsew', padx=10, pady=5)
        self.scr_computers.grid(row=1, column=1, sticky='nse', padx=(0, 10), pady=5)

        # Кнопки
        ttk.Button(
            self,
            text='Добавить клиента и заказ',
        ...
        self.btn_pdf.grid(row=2, column=1, sticky='nsew', padx=10, pady=10)

        # Настройка весов строк и столбцов для равномерного распределения
        self.rowconfigure(1, weight=1)  # Основное пространство для таблиц
    ...
    def get_full_info(self, data):
        ...
        try:
            lst = []
            lst.append(data[1])
            lst.append(f'{"Создан" if data[2] == 0 else "Закрыт"}')
...
class FullDataBaseWindow(ttk.Toplevel):
    ...
    def __init__(self):
        super().__init__()
        self.title('Учет заказов ПК - Режим редактора')
...
class AddMotherWindow(ttk.Toplevel):
    ...
    def __init__(self):
        super().__init__()
        self.title('Учет заказов ПК - Добавить материнскую плату')
...
class AddProcessorWindow(ttk.Toplevel):
    ...
    def __init__(self):
        super().__init__()
        self.title('Учет заказов ПК - Добавить процессор')
        self.geometry('600x200')
        self.resizable(False, False)
        self.iconbitmap('logo.ico')
...
class AddCompuerWindow(ttk.Toplevel):
    ...
    def __init__(self):
        super().__init__()
        self.title('Учет заказов ПК - Добавить заказ')
...
class AddFriendWindow(ttk.Toplevel):
    ...
    def __init__(self):
        super().__init__()
        self.title('Учет заказов ПК - Добавить клиента')
...
class EditMotherWindow(ttk.Toplevel):
    ...
    def __init__(self):
        super().__init__()
        self.title('Учет заказов ПК - Редактировать материнскую плату')
...
class EditProcessorWindow(ttk.Toplevel):
    ...
    def __init__(self):
        super().__init__()
        self.title('Учет заказов ПК - Редактировать процессор')
        self.geometry('600x300')
        self.resizable(False, False)
        self.iconbitmap('logo.ico')
...
class EditComputersWindow(ttk.Toplevel):
    ...
    def __init__(self):
        super().__init__()
        self.title('Учет заказов ПК - Редактор заказов')
        self.geometry('450x350')
        self.resizable(False, False)
        self.iconbitmap('logo.ico')
    ...
    def save(self):
        ...
        if not self.validator.validate_name_and_status(self.name):
            ...
            self.error.set('*Выберите заказ')
            self.success.set('')
        else:
            self.error.set('')
            self.success.set('Информация о заказе успешно обновлена')
...
class EditFriendWindow(ttk.Toplevel):
    ...
    def __init__(self):
        ...
        self.friend_var = ttk.StringVar(value='Выберите клиента')

        self.comp_id = None
        self.comp_item = self.get_item()[1]
        self.comp_var = ttk.StringVar(value='Выберите заказ')
        self.error = ttk.StringVar()
        self.success = ttk.StringVar()
        ...
        ttk.Label(
            self,
            text='Выберите его заказ в таблице:').grid(
            row=0,
            column=2,
            sticky='w',
            pady=5
        ...
        ttk.Label(self,
                  text='Добавить заказ клиенту:').grid(
            row=2,
            column=0,
            padx=10,
            sticky='w'
    ...
    def delete_friend(self):
        ...
        if not self.validator.validate_id(self.friend_id):
            ...
            self.error.set('')
            self.success.set('Клиент успешно удален')
            self.friend_obj.delete(self.friend_id)
            self.friend_var.set('Выберите клиента')
            self.comp_var.set('Выберите заказ')
            self.friend_id = None
            self.update_table()
            self.update_combo()
            self.update_editor()
    ...
    def add_comp_to_friend(self):
        ...
        if self.comp_id in ides:
            self.error.set('*Такой заказ есть у этого клиента')
            self.success.set('')
        else:
            ...
            self.error.set('')
            self.success.set('Заказ успешно добавлен клиенту')
            self.update_table()
...
if __name__ == '__main__':
    create_db()
    App('Учет заказов ПК', WINDOW_SIZE)
